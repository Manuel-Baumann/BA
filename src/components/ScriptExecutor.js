...
const radioGroupData = [
    ...
    },*/
    {
        ...
        info:
            "Choose wether to run the algorithm on the courses names or the grades that the students got. Putting values into bins means for " +
            "grades, 1.0, 1.3, 1.7 -> 1 ... - and for courses putting math subjects into the Math-Bin and so on.",
    },
    {
        groupName: "Time period / Basis for Itemsets",
        options: ["Year", "Semester"],
        info: "Choose wether to run the algorithm on a semester`s basis or a year`s basis.",
    },
    {
        groupName: "Insights",
        options: ["Normal", "Closed", "Maximal"],
        info: "Choose which kind of ouput you want",
    },
    {
        ...
        info:
            "Choose one of the three different types of information to be shown. Having Students as basis means not treating each" +
...
const radioGroupColumnData = [
    ...
    {
        header: "Exams",
        options: ["All", "Only passed exams", "Only failed exams"],
        info: "Only students who graduated at RWTH: Filtering for students who have passed all mandatory courses at RWTH, including the bachelor`s thesis.",
    },
    {
        options: ["All", "m", "w", "x"],
...
const infoMinMax =
    "Students will be sorted by their mean grade. Only those within the range (in %) will be considered.";
const infoSemesters =
    "Only courses taken within the specified range will be considered.";
...
const ScriptExecutor = () => {
    ...
    const [selectedCheckboxColumnValues, setSelectedCheckboxColumnValues] =
        useState({
    ...
    const [filterARNotOnlyMandatory, setFilterARNotOnlyMandatory] =
        useState(false);
    ...
    const [remainingGradesObj, setRemainingGradesObj] = useState(
        resetRemainingGrades()
    );
    ...
    const executeScript = async (columnIndex) => {
        ...
        const columnValues =
            columnIndex === 1
                ? selectedColumnValues.column1
                : selectedColumnValues.column2;
        const checkboxColumnData =
            columnIndex === 1
                ? selectedCheckboxColumnValues.column1
                : selectedCheckboxColumnValues.column2;
        const range = rangeValues[`column${columnIndex}`];
        const semesterRange = semesters[`column${columnIndex}`];
        let response = "";
        const binsBooleanForAlgorithm =
            values.at(0) === "Grades" ? binsBool : false;
        const minS1 = minSups.column1[0] ? minSups.column1[0] : minSups.column1;
        const minC1 = minConfs.column1[0]
            ? minConfs.column1[0]
            : minConfs.column1;
        const minS2 = minSups.column2[0] ? minSups.column2[0] : minSups.column2;
        const minC2 = minConfs.column2[0]
            ? minConfs.column2[0]
            : minConfs.column2;
        try {
            ...
            response = await axios.post("http://localhost:5000/execute", {
                ...
                checkBoxData: Object.keys(checkboxColumnData).filter(
                    (k) => checkboxColumnData[k]
                ),
                ...
                filterARResults: filterARNotOnlyMandatory,
            });
        } catch (error) {
        ...
        try {
            ...
            while (i < responseLines.length && i < 30) {
                ...
                if (
                    ...
                    responseLines[i].startsWith(" Pattern count")
                ) {
                    leaveIn = i;
                }
            ...
            if (first !== 0) {
                ...
                responseLines = responseLines
                    .slice(0, first)
                    .concat(responseLines[leaveIn].trim())
                    .concat(responseLines[leaveIn + 1])
                    .concat(responseLines.slice(last + 1));
            }

            // Get basis number of students/semesters (transactions) of algorithm
            const basisForSupportFromOutput = responseLines
                .filter((line) =>
                    line.startsWith("Unique students in used dataset")
                )[0]
                .split(":")[1]
                .trim();
            ...
            const mainOutput = responseLines
                .filter((line) => !line.startsWith('"""POSTPROCESSING"""'))
                .join("\n");
            const preprocOutput = responseLines
                .filter((line) => line.startsWith('"""POSTPROCESSING"""'))
            ...
            const index = mainOutputSplitted.findIndex((line) =>
                line.startsWith("OUTPUT:")
            );
            let onlyOutput = mainOutputSplitted
                .slice(index + 1)
                .filter((line) => line.trim() !== "");
            const preOutput = mainOutputSplitted
                .slice(0, index + 1)
                .filter((line) => line.trim() !== "");
            ...
            if (selectedValues.at(3) === "Sequence Patterns") {
                icicleData = buildIcicleHierarchySeqPats(
                    onlyOutput,
                    basisForSupportFromOutput,
                    amountOfUniqueStudentsInDatasetGLOBAL
                );
                lastExecuted = 2;
            } else if (selectedValues.at(3) === "Association Rules") {
                icicleData = buildAssRulesIcicleHierarchy(
                    onlyOutput.map((str) => str.replace(/==>/g, "=>")),
                    basisForSupportFromOutput,
                    amountOfUniqueStudentsInDatasetGLOBAL
                );
                lastExecuted = 1;
            } else if (selectedValues.at(3) === "Frequent Itemsets") {
                icicleData = buildFrequentItemsetHierarchy(
                    onlyOutput,
                    basisForSupportFromOutput,
                    amountOfUniqueStudentsInDatasetGLOBAL
                );
            ...
            if (columnIndex === 1) {
                ...
                setOutput1(errorMessage);
                const icicleData = buildFrequentItemsetHierarchy(
                    ["Empty dataset #SUP:1"],
                    1,
                    1
                );
    ...
    const compareOutputs = () => {
        if (lastExecuted1 === 0) {
            ...
            const split1 = outputLines1.map((l, i) => [
                l.split("#SUP:")[0].trim().split(" || ").sort().join(" && "),
                l.split("#SUP:")[1].trim(),
            ]);
            const split2 = outputLines2.map((l, i) => [
                l.split("#SUP:")[0].trim().split(" || ").sort().join(" && "),
                l.split("#SUP:")[1].trim(),
            ]);
            const setOfStrings1 = new Set(split1.map(([str]) => str));
            const setOfStrings2 = new Set(split2.map(([str]) => str));
            const setOfNonUniqueStrings = new Set(
                [...setOfStrings1].filter((item) => setOfStrings2.has(item))
            );
            const setUnique1 = new Set(
                [...setOfStrings1].filter((item) => !setOfStrings2.has(item))
            );
            const setUnique2 = new Set(
                [...setOfStrings2].filter((item) => !setOfStrings1.has(item))
            );
            const arrayOfNonUniqueObjects = [...setOfNonUniqueStrings].map(
                (item) => ({
                    label: item,
                    leftValue: split1.find(([str]) => str === item)?.[1],
                    rightValue: split2.find(([str]) => str === item)?.[1],
                    leftValueWithDifferentBasis: parseFloat(
                        (split1.find(([str]) => str === item)?.[1] *
                            basisForSupportLeft) /
                            amountOfUniqueStudentsInDatasetGLOBAL
                    ),
                    rightValueWithDifferentBasis: parseFloat(
                        (split2.find(([str]) => str === item)?.[1] *
                            basisForSupportRight) /
                            amountOfUniqueStudentsInDatasetGLOBAL
                    ),
                })
            );
            ...
            const uniqueToColumn1 = split1.filter(
                ([str]) => !setOfStrings2.has(str)
            );
            const uniqueToColumn2 = split2.filter(
                ([str]) => !setOfStrings1.has(str)
            );

            const o1 = uniqueToColumn1.map(
                ([str, sup]) => `${str}       Support: ${sup}`
            );
            const o2 = arrayOfNonUniqueObjects.map(
                (item) =>
                    `${item.label}       Support: left->${item.leftValue}  right->${item.rightValue}`
            );
            const o3 = uniqueToColumn2.map(
                ([str, sup]) => `${str}       Support: ${sup}`
            );
        ...
        // Association Rules
            ...
            const split1 = outputLines1.map((l, i) => [
                l
                    ...
                    .map((side) => side.split(" || ").sort().join(" && "))
                    .join(" => "),
                l.split("#SUP:")[1].split("#CONF:")[0].trim(),
                l.split("#CONF:")[1].trim(),
            ]);
            const split2 = outputLines2.map((l, i) => [
                l
            ...
            const arrayOfNonUniqueObjects = [...setOfNonUniqueStrings].map(
                (item) => ({
                    ...
                    leftConfidenceWithDifferentBase: parseFloat(
                        (split1.find(([str]) => str === item)?.[2] *
                            basisForSupportLeft) /
                            amountOfUniqueStudentsInDatasetGLOBAL
                    ),
                    rightConfidenceWithDifferentBase: parseFloat(
                        (split2.find(([str]) => str === item)?.[2] *
                            basisForSupportRight) /
                            amountOfUniqueStudentsInDatasetGLOBAL
                    ),
                })
    ...
    const handleFilterARResultsChange = () => {
        setFilterARNotOnlyMandatory((prev) => !prev);
    };
    ...
    useEffect(() => {
        if (remainingGradesObj[0]) {
            setCurrentBin(remainingGradesObj[0]);
        }
    }, [remainingGradesObj]);
    ...
    const getBinOfItem = (item, index) => {
        ...
        const firstIndex =
            index === 0
                ? 0
                : allGrades.findIndex((el) => el === binsArr[index - 1]) + 1;
    ...
    const renameOutputToBinsName = (data) => {
        ...
        return data.map((str) => {
            ...
            return supportAndConfidence
                ? `${updatedRule}${supportAndConfidence}`
                : updatedRule;
        });
    };
    ...
    return (
        <div className="container">
            ...
            <div className="radio-group-container">
                {radioGroupData.map((group, groupIndex) => (
                    <div key={groupIndex} className="radio-group">
                        ...
                        {group.options.map((option, optionIndex) => (
                            <label key={optionIndex}>
                                <input
                                    ...
                                    value={option}
                                    checked={
                                        selectedValues[groupIndex] === option
                                    }
                                    onChange={() =>
                                        handleGroupSelection(groupIndex, option)
                                    }
                                    disabled={
                                        group.groupName ===
                                            "Time period / Basis for Itemsets" &&
                                        selectedValues.at(3) !==
                                            "Sequence Patterns" &&
                                        studentsBasis
                                    }
                        ...
                        {group.groupName ===
                            "Time period / Basis for Itemsets" &&
                        selectedValues.at(3) !== "Sequence Patterns" ? (
                        ...
                        {group.groupName === "Type of academic data" ? (
                            <div key="OnlyMandatoryBool">
                                <label>
                                    <input
                                        type="checkbox"
                                        name="Categorise: Mandatory / Optional"
                                        checked={onlyMandatoryBool}
                                        onChange={handleOnlyMandatoryBoolChange}
                                    />
                                    Categorise: Mandatory / Optional
                                </label>
                            </div>
                        ) : null}

                        {group.groupName === "Postprocessing" ? (
                            <div className="checkbox-container">
                                {/* Slider for Number of ouput lines */}
                                <div className="slider-container">
                                    <label>
                                        Output size: {numberOfOutputLines}{" "}
                                        <span className="info-icon">
                                            <span className="tooltip-text">
                                                Amount of lines of output to be
                                                vizualized (200 -&gt; All
                                                produced output)
                                            </span>
                                        </span>
                                    ...
                                    <Slider
                                        ...
                                        onChange={(value) =>
                                            setNumberOfOutputLines(value)
                                        }
                                    />
                                </div>

                                {selectedValues.at(3) === "Frequent Itemsets" &&
                                selectedValues.at(0) === "Courses" &&
                                selectedValues.at(2) == "Normal" ? (
                                    <div className="checkbox-container">
                                        <div key="FI filter only optional">
                                            <label>
                                                <input
                                                    ...
                                                    checked={
                                                        filterFIOnlyOptional
                                                    }
                                                    onChange={
                                                        handleFilterFIResultsChange
                                                    }
                                                />
                                                Filter results: Only
                                                non-mandatory
                                ...
                                {selectedValues.at(3) === "Association Rules" &&
                                selectedValues.at(0) === "Courses" &&
                                selectedValues.at(2) == "Normal" ? (
                                    <div className="checkbox-container">
                                        <div key="AR filter only optional">
                                            <label>
                                                <input
                                                    type="checkbox"
                                                    name="AR filter only optional"
                                                    checked={
                                                        filterARNotOnlyMandatory
                                                    }
                                                    onChange={
                                                        handleFilterARResultsChange
                                                    }
                                                />
                                                Filter results: Only containing
                                                optional
                                            </label>
                                        </div>
                                    </div>
                                ...
                                <div key="Put values into bins">
                                    <label>
                                        ...
                                        <span className="info-icon">
                                            ℹ️
                                            <span className="tooltip-text">
                                                By choosing bins, you tell the
                                                algorithm to treat values as if
                                                they were different ones. For
                                                example, the default bins put
                                                '1.0', '1.3', '1.7' into the bin
                                                named '1', meaning they are
                                                treated as if they were the
                                                value '1'. You can rename the
                                                bins.{" "}
                                            </span>
                                        </span>
                                    </label>
                                    {binsBool ? (
                                        <>
                                            <GradeLine
                                                addBinHandler={addBinHandler}
                                                handleDeleteBin={
                                                    handleDeleteBin
                                                }
                                                binsArr={binsArr}
                                                setBinsArr={setBinsArr}
                                                allGrades={allGrades}
            ...
            <div className="columns-container">
                <div className="column">
                    ...
                    <div className="checkbox-container">
                        <h3>
                            ...
                            <span className="info-icon">
                                ...
                                <span className="tooltip-text">
                                    <h4>
                                        If you exclude a group of students, they
                                        won't be considered by the algorithm.
                                        Don't exclude two groups that won't
                                        leave any students for the algorithm to
                                        run on.
                                    </h4>
                                </span>
                            </span>
                            ...
                            <div key={item}>
                                <label>
                                    <input
                                        ...
                                        name={item}
                                        checked={
                                            selectedCheckboxColumnValues
                                                .column1[item]
                                        }
                                        onChange={(e) =>
                                            handleCheckboxColumnValueChange(
                                                e,
                                                1
                                            )
                                        }
                    ...
                    <div className="slider-container">
                        <label>
                            Semesters: Min:{" "}
                            {semesterToTerm(semesters.column1[0])} | Max:{" "}
                            {semesterToTerm(semesters.column1[1])}
                            <span className="info-icon">
                                ℹ️
                                <span className="tooltip-text">
                                    {infoSemesters}
                        ...
                        <Slider
                            ...
                            onChange={(values) =>
                                handleSemesterChange(values, 1)
                            }
                        />
                        <label>
                            Students mean grades: Min: {rangeValues.column1[0]}{" "}
                            | Max: {rangeValues.column1[1]}
                            <span className="info-icon">
                                ℹ️
                                <span className="tooltip-text">
                                    {infoMinMax}
                    ...
                    <div>
                        <div className="slider-container">
                            <label>
                                Minimum support: {minSups.column1}%{" "}
                                <span className="info-icon">
                                    ℹ️
                                    <span className="tooltip-text">
                                        {" "}
                                        Choose the minimum support for the
                                        frequent itemsets{" "}
                                    </span>
                                </span>{" "}
                            </label>
                    ...
                    <div>
                        ...
                        <pre className="pre">{postProcOutput1}</pre>{" "}
                        {/* Extra output for column 1 */}
                        <h3>Information about the algorithm:</h3>
                        <pre className="pre">{preOutput1}</pre>{" "}
                        {/* Extra output for column 1 */}
                        {/* Graph */}
                        {lastExecuted1 !== 0 ? (
                            lastExecuted1 === -1 ? (
                                ...
                                >
                                    <IcicleWithHover
                                        data={data1}
                                        setData={setData1}
                                        basisForSupport={basisForSupportLeft}
                                    />
                                </div>
                            )
                        ) : (
                ...
                <div className="column">
                    ...
                    <div className="slider-container">
                        <label>
                            Semesters: Min:{" "}
                            {semesterToTerm(semesters.column2[0])} | Max:{" "}
                            {semesterToTerm(semesters.column2[1])}
                            <span className="info-icon">
                                ℹ️
                                <span className="tooltip-text">
                                    {infoSemesters}
                        ...
                        <Slider
                            ...
                            onChange={(values) =>
                                handleSemesterChange(values, 2)
                            }
                        />
                        <label>
                            Students mean grades: Min: {rangeValues.column2[0]}{" "}
                            | Max: {rangeValues.column2[1]}{" "}
                            <span className="info-icon">
                                ℹ️
                                <span className="tooltip-text">
                                    {infoMinMax}
                    ...
                    <div>
                        <div className="slider-container">
                            <label>
                                Minimum support: {minSups.column2}%{" "}
                                <span className="info-icon">
                                    ℹ️
                                    <span className="tooltip-text">
                                        {" "}
                            ...
                            <Slider
                                ...
                                onChange={(value) =>
                                    handleMinSupChange(value, 2)
                                }
                            />
                            {selectedValues.at(3) === "Association Rules" ? (
                                <div>
                                    <label>
                                        Minimum confidence: {minConfs.column2}%{" "}
                                        <span className="info-icon">
                                            ℹ️
                                            <span className="tooltip-text">
                                                {" "}
                    ...
                    <div>
                        <h3>Postprocessing:</h3>
                        <pre className="pre">{postProcOutput2}</pre>{" "}
                        {/* Extra output for column 2 */}
                        <h3>Information about the algorithm:</h3>
                        <pre className="pre">{preOutput2}</pre>{" "}
                        {/* Extra output for column 1 */}
                        {/* Graph */}
                        {lastExecuted2 !== 0 ? (
                            lastExecuted2 === -1 ? (
                                ...
                                >
                                    <IcicleWithHover
                                        data={data2}
                                        setData={setData2}
                                        basisForSupport={basisForSupportRight}
                                    />
                                </div>
                            )
                        ) : (
                ...
                <>
                    ...
                    <div className="compare-output">
                        <pre style={{ width: "20%" }}>
                            <h4>
                                ...
                                <span className="info-icon">
                                    ...
                                    <span className="tooltip-text">
                                        {" "}
                                        This graph shows itemsets and their
                                        support. You can hover over the itemsets
                                        and get information about them.{" "}
                                    </span>
                                </span>
                            </h4>
                            {diffOutputLeft && diffOutputLeft.length > 0 ? (
                                <div className="compare-container">
                                    {/*diffOutputLeft*/}
                                    <BarChartWithTransitions
                                        data={diffLeftData}
                                        sizeOfData={sizeOfDiffLeftData}
                                        setData={setDiffLeftData}
                                        basisForSupport={basisForSupportLeft}
                                    />
                                </div>
                            ) : (
                        ...
                        <pre style={{ width: "60%" }}>
                            <h4>
                                ...
                                <span className="info-icon">
                                    ...
                                    <span className="tooltip-text">
                                        {" "}
                                        This graph shows itemsets, rules or
                                        patterns that exist in both the output
                                        on the left cohort AND the right cohort.
                                        Blue bars show the difference in support
                                        between the cohorts. Hover over the bars
                                        to find out information on the itemsets.
                                    </span>
                                </span>
                            </h4>
                            {diffOutputMiddle ? (
                        ...
                        <pre style={{ width: "20%" }}>
                            ...
                            {diffOutputRight && diffOutputRight.length > 0 ? (
                                <div className="compare-container">
                                    {/*diffOutputRight*/}
                                    <BarChartWithTransitions
                                        data={diffRightData}
                                        sizeOfData={sizeOfDiffRightData}
                                        setData={setDiffRightData}
                                        basisForSupport={basisForSupportRight}
                                    />
                                </div>
                            ) : (
            ...
            ) : lastExecuted1 === lastExecuted2 && lastExecuted1 != -1 ? (
                <button className="compare-button" onClick={compareOutputs}>
                    Show the difference between columns
                </button>
            ) : (
                <></>
            )}
